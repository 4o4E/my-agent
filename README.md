...
- Node.js ≥ 20
- pnpm 11.x（可通过 `corepack enable` 使用 `package.json` 中声明的版本）
...
脚本入口位于 `scripts/`，适合 Linux 开发机或服务器直接管理前后端开发进程：

```bash
pnpm run start    # 后台启动后端 + 前端
pnpm run stop     # 停止整组进程
pnpm run restart  # 重启整组进程
```

- PID 文件：`.run/my-agent.pid`
- 日志文件：`logs/my-agent.log`
- 启动脚本会检查 pnpm 依赖是否已安装；首次使用前仍需执行 `pnpm install`。
...
```bash
pnpm --filter server test       # node:test：工具 / provider 协议映射 / 执行循环
