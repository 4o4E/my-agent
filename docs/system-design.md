...
这个取舍让原型更容易本地启动和迭代，但也意味着它还不是完整云端多 worker 平台。独立队列、worker 调度、重启续跑、多租户鉴权和资源配额属于后续平台化能力。
...
- 支持上下文压缩、Goal 锚点、取消 run、工具输出截断和工具策略。
...
- ContextManager 在每次 LLM 调用前检查 token 预算，必要时做 observation masking 或滑动窗口压缩。
...
- `compaction.ts`：上下文压缩纯函数，包括 token 估算、tool result masking、滑动窗口。
...
- `providers/openaiResponses.ts`、`openaiChat.ts`、`anthropic.ts`：手写协议 provider，作为兼容和回退路径。
...
- `openai-responses`：手写 OpenAI Responses API provider，保留为回退路径。
- `openai-chat`：手写 Chat Completions provider，可兼容部分 OpenAI-compatible 服务。
- `anthropic`：手写 Anthropic Messages provider。
...
- `messages.collapsed='summarized'` 预留给后续 L3 摘要。
...
- run 级 lease、心跳、崩溃恢复和重启续跑。
- 多租户鉴权、用户隔离、项目隔离和审计权限。
- per-run 独立工作区生命周期管理。
- CPU、内存、进程数、磁盘和网络资源配额。
- `ask_user` 真正暂停 run 并等待用户回答。
- L3 锚定摘要、重复工具调用检测和无进展检测。
